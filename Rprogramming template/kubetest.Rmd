...
Captain John $\hat{\lambda}=1.02$ Sheridan: You know, I just had a thought. You've been back and forth to your world so many $\int_{0}^{12}\frac{x_{d}^{2}}{2}+y^{2}ds$
...
\[
\begin{array}{cc}
minimize & f_{0}\left(x_{1},x_{2}\right)\\
subject\,to & 2x_{1}+x_{2}\geq1\\
 & x_{1}+3x_{2}\geq1\\
 & x_{1}\geq0,\,x_{2}\geq0.
\end{array}
\]
For each of the following objective functions, find a solution of
the problem.
#item
    dd
\begin{enumerate}
...
In a Boolean linear program, the variable $x$ is constrained to have
components equal to zero or one:
...
\end{array}
\]
In general, such problems are very difficult to solve, even though
the feasible set is finite (containing at most $2^{n}$ points). In
a general method called relaxation, the constraint that be zero or
one is replaced with the linear inequalities :
\[
\begin{array}{cc}
minimize & c^{T}x\\
subject\,to & Ax\preceq b\\
 & 0\leq x_{i}\leq1,\,i=1,...,n.
\end{array}
\]
We refer to this problem as the LP relaxation of the Boolean LP. The
LP relaxation is far easier to solve than the original Boolean LP.
\begin{enumerate}
...
This exercise builds on exercises 4.15 and 5.13 in Convex Optimization(textbook),
which involve the Boolean LP
\[
\begin{array}{cc}
minimize & c^{T}x\\
subject\,to & Ax\preceq b\\
...
\end{array}
\]
with optimal value $p^{\star}$. Let $x^{rlx}$ be a solution of the
LP relaxation
\[
\begin{array}{cc}
minimize & c^{T}x\\
...
\end{array}
\]
so $L=c^{T}x^{rlx}$ is a lower bound on $p^{\star}$. The relaxed
solution $x^{rlx}$ can also be used to guess a Boolean point $\hat{x}$,
by rounding its entries, based on a threshold $t\in\left[0,1\right]$:
\[
...
Evidently $\hat{x}$ is Boolean (i.e., has entries in $\left\{ 0,1\right\} $).
If it is feasible for the Boolean LP, i.e., if $A\hat{x}\preceq b$,
then it can be considered a guess at a good, if not optimal, point
for the Boolean LP. Its objective value, $U=c^{T}\hat{x}$, is an
upper bound on $p^{\star}$. If $U$ and $L$ are close, then $\hat{x}$
is nearly optimal; specifically, $\hat{x}$ cannot be more than $\left(U-L\right)$-suboptimal
for the Boolean LP. This rounding need not work; indeed, it can happen
that for all threshold values, is infeasible. But for some problem
instances, it can work well. Of course, there are many variations
on this simple scheme for (possibly) constructing a feasible, good
point from $x^{rlx}$.
...
\end{description}
After updating the information to solve the optimization problem,
use the $\mathtt{solve}$ commend to calculate the solution.

The return value $0$ illustrates that the problem is feasible. The
optimal point and optimal value can be obtained by using $\mathtt{get.variables}$
and $\mathtt{get.objective}$. 
...
The result shows that the number of decision variable chosen was \Sexpr{length(optp[optp>0])},
and among these variables, there are \Sexpr{length(optp[optp>0])-length(optp[optp==1])}
variables are not integer, which makes the optimal point infeasible
in Boolean LP. Since the optimal value of relaxation LP is a lower
bound of the optimal value of Boolean LP, we set the variable $\mathtt{L}=c^{T}x^{rlx}=$\Sexpr{L}. 

Now, we will find out the best approximation of the optimal point
of LP. To find it, we need to calculate the upper bound $U=c^{T}\hat{x}$
first. Carry out threshold rounding for $100$ values of $t$, uniformly
spaced over $\left[0,1\right]$. For each value of $t$, calculate
$\hat{x}$ from above and keep the the objective value $c^{T}\hat{x}$
and the maximum constraint violation $max\left(A\hat{x}-b\right)$.
The best approximation of the optimal point of the LP can be obtained
by finding the $\hat{x}$ which makes the gap of $\left(U-L\right)$
to be a minimum.
...
\end{cases}
\]
for each $t$. Next, the maximum constraint violation $max\left(A\hat{x}-b\right)$
and the gap between $U-L$ can be calculated as follows:
```{r echo=TRUE,message=FALSE,warning=FALSE}
...
The best approximation of the optimal point of LP, $\hat{x}_{best}$,should
not only make the gap to be minimum, but also be feasible in LP. In
other words, $\hat{x}_{best}$ should not violate the constraints;
$max\left(A\hat{x}_{best}-b\right)\leq0$. Thus, the last line of
code determine $\hat{x}_{best}$ under the constraints, and the optimal
value of the Boolean LP, $\hat{p}^{\star}$, is approximately equal
to \Sexpr{t(c) %*% best_x_hat}. This approximation fairly good because
the gap between $U-L$ was \Sexpr{gap[min(which(maxerror<0))]}.
```{r echo = FALSE, fig.align='center', fig.width = 10, fig.height = 5}
old.par <- par(mfrow=c(1, 2))
t <- seq(0, 1, length.out = 100)
plot(t, gap, xlim = c(0,1), ylim = c(-14,8), type="l", xlab="t", ylab="gap: U-L")
abline(h = seq(-10, 5, length.out = 4),
	   v = seq(0, 1, length.out = 6),
	 col = "lightgray", lty=3)
abline( v = 0,lty=1)
abline( h = 0,lty=1)

plot(t, maxerror, xlim = c(0,1), ylim = c(-5,10), type="l", xlab="t", ylab="max error")
abline(h = seq(-5, 30, length.out = 8),
 	  v = seq(0, 1, length.out = 6),
	 col = "lightgray", lty=3)
abline( v = 0,lty=1)
abline( h = 0,lty=1)
